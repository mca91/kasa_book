...
Eine Konsequenz falscher Standardfehler kann eine Verletzung der nominalen $\alpha$-Fehlerrate in Signifikanztests sein: Der Hypothesentest lehnt eine korrekte Nullhypothese mit einer geringeren oder höheren Fehlerwahrscheinlichkeit (das Signifikanzniveau $\alpha$) als gewünscht ab. 

Inwiefern die für die Simulation gewählte Form von Heteroskedastizität die $\alpha$-Fehlerrate eines t-Tests beeinflusst, können wir anhand der Ergebnisse schätzen: Wir berechnen die Ablehnrate für t-Tests der (wahren) Null-Hypothese $H_0:\beta_1=0$ zum 5\%-Niveau jeweils für robuste und nicht-robuste Standardfehler und vergleichen.
...
1. **Stichproben mit Zurücklegen ziehen**: Ziehe wiederholt ($B$ mal) Stichproben der Größe $N$ (Größe der ursprünglichen Stichprobe) mit Zurücklegen aus den Originaldaten.
...
3. **Verteilungsparameter Schätzen**: Anhand der $B$ Bootstrap-Schätzungen aus Schritt 2 können die Eigenschften der Verteilung des Schätzers $\widehat{\beta}$ oder Funktionen davon geschätzt werden:

    - Das arithmetische Mittel der $B$ Bootstrap-Schätzungen ist ein Schätzer des Erwartungswerts von $\widehat{\beta}$ 
    - Der Standardfehler des Schätzers kann durch die Standardabweichung der $B$ Bootstrap-Schätzungen geschätzt werden
    - Ein 95\%-Bootstrap-Konfidenzintervall kann mit den 2.5\%- und 97.5\%-Perzentilen berechnet werden werden

### Interaktive Visualisierung des Bootstraps

Die nachfolgende interaktive Visualisierung illustriert die Vorgehensweise des oben beschriebenen Bootstrap-Algorithmus für die Schätzung eines 95\%-Konfidenzintervalls für den Erwartungswert einer normalen <span style="color: purple;">Verteilung</span> basierend auf einer <span style="color: red;">Stichprobe</span>, wenn das arithmetische Mittel als Schätzer verwendet wird. Diese <span style="color: purple;">Pupulationsverteilung</span> ist $N(\mu = 50, \sigma^2 = 12^2)$. In jeder Iteration des Bootstraps wird eine <span style="color: orange;">Bootstrap-Stichprobe</span> mit Zurücklegen aus der originalen <span style="color: red;">Stichprobe</span> gezogen. Für jede <span style="color: orange;">Bootstrap-Stichprobe</span> berechnen wird das <span style="color: darkgreen;">Arithmetische Mittel (grüne Punkte)</span>. Für hinreichend viele <span style="color: darkgreen;">Bootstrap-Schätzungen</span> erhalten wir eine <span style="color: darkgreen;">Bootstrap-Verteilung</span> unseres Schätzers.^[Die Grüne Linie zeigt eine Kerndichte-Schätzung der Bootstrap-Verteilung. Die Bandweite dieser Schätzung kann in der Applikation variiert werden.] Basierend auf dieser Verteilung erhalten wir eine Bootstrap-Schätzung des Erwartungswerts (schwarzer Punkt) und ein 95\%-Konfidenzintervall (schwarze Linie).

In der interaktiven Grafik können wir insbesondere folgende Eigenschaften des Bootstrap-Verfahrens verifizieren:

- Größere <span style="color: red;">Stichproben</span> verringern die Unsicherheit des interessierenden Schätzers, sodass die Varianz seiner Verteilung nimmt mit der Stichprobengröße abnimmt. Da der Bootstrap die Verteilung des Schätzers approximiert, spiegelt sich diese Eigenschaft auch in der <span style="color: darkgreen;">Bootstrap-Verteilung</span>.
