...
@tbl-ADD_dat gibt einen Überblick der verfügbaren Variablen in `ADD_dat`.

```{r, echo = F}
#| tbl-cap: "`ADD_dat` -- Charakteristika von Italienischen Gemeinden im 19. Jahrhundert"
#| label: tbl-ADD_dat
tibble(
  Variable = colnames(ADD_dat),
  Beschreibung = c(
    "Präsenz Fasci in 1893-Q4 (Dummy)",
    "Relative Regenenge (Frühlung 1893, mm)",
    "Nächste aktive Wetterstation (Frühlung 1893)",
...
Vor der Analyse filtern wir die Daten entsprechen der Vorgehensweise in @@Acemogluetal2020: Es werden nur Gemeinden berücksichtigt, die in maximal 30km Entfernung zu einer Wetter-Station liegen (`!is.na(sp3m1893_n30)`) *und* für die eine Messung der Mafia-Aktivität gegen Ende des 19. Jahrhunderts vorliegt (`!is.na(Mafia1900)`).
