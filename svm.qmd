...
Wir trainieren nun einen MMC für `klasse` in `lsvm_data` mit `e1071::svm()`. Mit `kernel = "linear"` legen wir fest, dass eine lineare Entscheidungsgrenze im zwei-diemensionalen Raum von $(X_1,X_2)$ gefunden werden soll. Die Parameter `cost = 1e8` und `scale = F` definieren eine harte Margin (mehr dazu im Abschnitt zu SVMs unten) sowie keine Skalierung der Daten. Mit `coef()` werden die trainierten Koeffizienten ausgelesen.
...
ggplot(
  ...
  mapping = aes(x = x1, y = x2, color = klasse)
    ...
    labs(
      x = "X1", 
      y = "X2", 
      title = "Max. Margin Classifier: Trennebene, Margin und SV"
    ) +
    # Trennlinie
    geom_function(
...
Die nachfolgende Grafik illustriert die Empfindlich des MMC durch Hinzufügen *nur einer* zusätzlichen ungewöhnlichen Beobachtung der Klasse 1 mit $X_1 = -.5$ und $X_2 = 3$^[Der Code hierfür ist `lsvm_data %>% add_row(tibble(x1 = -.5, x2 = 3, klasse = factor(1)))`.]: Das Optimierungskalkül des MMC führt zu einer deutlich verschobenen Trennlinie im Vergleich zu ABZ.
...
Ein MMC kann für `lsvm_nonsep_data` keine Lösung finden: Der Algorithmus findet keine Lösung für das MMC-Optimierungsproblem und iteriert bis zu einer maximalen Anzahl an Durchläufen.

```{r}
#| error: true
# Versuch, MMC anzupassen Modell
model_mmc_nonsep <- svm(
  klasse ~ x1 + x2, 
  data = lsvm_nonsep_data, 
  kernel = "linear", 
  cost = 1e7,
...
Ein SVC hingegen kann angepasst werden: Mit `cost = 1` räumen wir ein  "Budget" für Verletzungen der Margin und Fehlklassifikationen ein.^[Der Parameter `cost` in `svm()` ist ein Langrange-Multiplikator und verhält sich daher invers zu $C$ in \eqref{eq:svcreg}: Große Werte für `cost` führen zu viel Regularisierung (kleine Margin) und umgekehrt.]
...
Analog wie für `model_mmc` können wir die trennende Gerade gemeinsam mit den Margins sowie den Support-Vektoren graphisch darstellen.
...
ggplot(
  ...
  mapping = aes(x = x1, y = x2, color = klasse)
    ...
    labs(
      x = "X1", 
      y = "X2", 
      title = "Soft Margin Support Vector Classifier"
    ) +
    # Trennlinie
    geom_function(
      fun = \(x) alpha0 + alpha1 * x, col = "black"
    ...
    theme(legend.position = "top")
```
Aufgrund der *Soft Margin* des SVC werden hier Fehlklassifikationen in Kauf genommen. Beachte, dass sowohl Beobachtungen auf und innerhalb der Margin als auch *fehlklassifizierte* Beobachtungen die Support-Vektoren der trennenden Geraden sind. 
...
In empirischen Anwendungen gibt es selten exakt lineare Entscheidungsgrenzen. Wir diskutieren nun das Beispiel einer nicht-linearen Entscheidungsgrenze für SVC eine hilfreiche Approximation der Entscheidungsgrenze liefern kann und diskutieren anschließend eine Generalisierung zur Anpassung nicht-linearer Grenzen.

Der Datensatz `svm_wavy` enthält gleichverteilte Beobachtungen $(X_1, X_2)$ die gemäß der sinuidalen Entscheidungsgrenze
...
Zunächst plotten wir die Beobachtungen in `svm_wavy` gemeinsam mit der deterministischen Entscheidungsgrenze $g(x_1)$.
...
ggplot(
  ...
  labs(
    x = "X1",
    y = "X2", 
    title = "Beobachtungen mit nicht-linearer Entscheidungsgrenze"
  ) +
  geom_function(
    fun = \(x) x + 3 * sin(x * 1.2),
    col = "black"
...
Ein SVC kann die nicht-lineare Entscheidungsgrenze nicht abbilden, erreicht jedoch aufgrund der groben Teilbarkeit des Regressorraum in beide Klassen eine akzeptable Fehlklassifikationrate. Wir zeigen dies mit R.
...
grid <- grid %>%
  ...
  mutate(
    klasse = predict(svc_nonsep, grid)
  )
```
...
```{r}
# Anteil falscher Klassifzikationen für svm_wavy
1 - mean(predict(svc_nonsep) == svm_wavy$klasse)
```
...
Im nächsten Schritt berechnen wir für diese transformierten Daten `X_transformed` unter Verwendung der Klassen in `data_concentric$klasse` einen SVC, also eine SVM mit linearem Kernel: Wir suchen eine trennende Ebene für die transformierten Daten `X_transformed` in $\mathbb{R}^3$. 
...
Die interaktive @fig-polytrans3d zeigt die transformierten Datenpunkte in $\mathbb{R}^3$, die mit dem Modell-Objekt `svm_model_poly_trans` berechnete trennende Hyperebene (grün) und die Support-Vektoren (grau hinterlegt). 
...
Beachte, dass die grüne Hyperebene die die Daten nahezu perfekt trennt:
...
Ahand von Klassifizierungen im Prädiktorraum $\mathbb{R}^2$ mit `svm_model_poly` können wir die angepasste kreisförmige Entscheidungsgrenze visualisieren.
...
ggplot(
  ...
  geom_point(grid, mapping = aes(x = x1, y = x2, color = klasse), alpha = .05) +
  geom_point() +
  # Wahre Entscheidungsgrenze
  ggforce::geom_circle(
    mapping = aes(x0 = 0, y0 = 0, r = 1), color = "black"
  ) +
...
model_svm_linear <- svm(klasse ~ ., data = svm_wavy, kernel = "linear", scale = F)
```

```{r}
# Erzeuge Grid für die Vorhersagen
grid <- expand_grid(
  x1 = seq(min(svm_wavy$x1), max(svm_wavy$x1), length.out = 150),
  x2 = seq(min(svm_wavy$x2), max(svm_wavy$x2), length.out = 150)
) 
...
grid <- grid %>%
  # Vorhersagen auf dem Grid
  mutate(
    klasse = predict(model_svm_linear , grid)
  )
```
...
ggplot() +
  ...
  geom_point(
    data = svm_wavy,
    mapping = aes(x = x1, y = x2, color = klasse)) +
  labs(title = "RBF-SVM: Trainierte Entscheidungsgrenze") +
  # deterministische Entscheidungsgrenze
  geom_function(
    fun = \(x) x + 3 * sin(x * 1.2),
    col = "black"
...
ggplot() +
  ...
  labs(title = "RBF-SVM: Overfitting der Entscheidungsgrenze") +
  scale_x_continuous("X1", expand = c(0, 0)) +
  scale_y_continuous("X2", expand = c(0, 0)) +
  theme_cowplot() +
  theme(legend.position = "top")
