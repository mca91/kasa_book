...
RMarkdown ist ein Dokumentationsformat im R-Ökosystem, das es Nutzern ermöglicht die oben genannten Anforderungen an reprozierbare Forschung zu erfüllen. Es kombiniert die Funktionalitäten von [Markdown](https://markdown.de/), einer schlanken [Markup-Sprache](https://en.wikipedia.org/wiki/Markup_language) für Textformatierung, mit der Möglichkeit, direkt in R geschriebenen Code auszuführen und die Ergebnisse, einschließlich Tabellen und Grafiken in ein Ziel-Dokument einzubinden.
...
Nach erstmaligem abspeichern der `.Rmd`-Datei (hier unter dem namen `diamanten.rmd`) kann das Dokument kompiliert werden. Hierzu klicken wir in der Aktionsleiste unterhalb des Dokumenten-Tabs auf den Button "*Knit*", siehe @fig-rmdgifkomp.^[Eine Keyboard-Shurtcut zum Kompilieren der `.Rmd`-Datei ist `CMD + Shift + k` in MacOS und `STRG + Shift + k` bei Windows.] 
...
Eine `.rmd`-Datei mit dem obigen Code kann [hier](https://github.com/mca91/kasa_book/blob/main/misc/diamonds.rmd) herunterlagen werden. Alternativ kann in RStudio ein neues Dokument erstellt werden (siehe @sec-genrmd) und der Inhalt kopiert und eingefügt werden. 
...
Ein weiteres leistungsstarkes Werkzeug zur Förderung der Reproduzierbarkeit ist das *Shiny*-Paket, das es Forschenden ermöglicht, interaktive Webanwendungen und Dashboards zu erstellen. Diese Anwendungen können dynamische Visualisierungen und benutzerfreundliche Oberflächen bieten, wodurch komplexe Analysen verständlicher und zugänglicher gemacht werden.
