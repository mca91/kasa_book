...
# Random Forest: Berechne die Vorhersagen für behandelte und unbehandelte Gruppen
rf.predictions <- predict(rf)
tau.rf <- mean(rf.predictions[B == 1]) - mean(rf.predictions[B == 0])
cat("Geschätzter Behandlungseffekt mit Random Forest:", tau.rf, "\n")
```
...
# MSE für Random Forest bei verwendung von ATE
mse.rf <- sqrt(mean((tau.rf - tau)^2))
cat("MSE des Random Forest:", mse.rf, "\n")
```
...
```{r}
sqrt(mean((predict(object = rf, newdata = nl_effects_test %>% select(-Y, -tau)) - nl_effects_test$tau)^2))
```
...
```{r}
# ATE RF pred
predict(object = rf, newdata = nl_effects_test %>% select(-Y, -tau)) %>% mean()
```
