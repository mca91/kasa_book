...
In R erstellen wir die Spezifikation mit `parsnip::rand_forest()`. Der Parameter `mtry` legt fest, wie viele Regressoren zufällig für jeden Split zur Verfügung stehen. Wir nutzen den Algorithmus aus dem `randomForest`-Paket und legen in `set_engine()` fest, dass die von `randomForest::randomForest()` berechnete Variable-Importance-Metrik im Output-Objekt ausgegeben wird. Um die Spezifikation für verschiedene Werte von `mtry` anwenden zu können, implementieren wir die Spezifikation innerhalb einer Wrapper-Funktion `rf_spec_mtry()`. Mit `purrr::map()` iterieren wir `rf_spec_mtry()` über drei verschiedene Werte für den Tuning-Parameter `mtry`.
...
# Ausgabe der Liste von Modellen
rf_fits
```
